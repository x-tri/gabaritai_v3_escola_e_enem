...
pkill -9 -f "python.*python_omrchecker_service.*app.py" 2>/dev/null
...
echo ""
echo "📌 PASSO 2: Iniciando OMRChecker Service v2.0 (porta 5002)..."
echo "=========================================="

cd "$BASE_DIR/python_omrchecker_service"

# Criar venv se não existir
if [ ! -d "venv" ]; then
    echo -e "${YELLOW}Criando ambiente virtual Python OMRChecker...${NC}"
    python3 -m venv venv
    source venv/bin/activate
    pip install -q -r requirements.txt
    pip install -q flask pdf2image opencv-python opencv-contrib-python
else
    source venv/bin/activate
fi

# Iniciar serviço OMRChecker em background
nohup python app.py > /tmp/omr_service.log 2>&1 &
OMR_PID=$!

echo -e "${BLUE}PID OMRChecker: $OMR_PID${NC}"
sleep 3

# Verificar se iniciou
if check_port 5002; then
    echo -e "${GREEN}✅ OMRChecker v2.0 rodando na porta 5002${NC}"
else
    echo -e "${RED}❌ ERRO: OMRChecker Service não iniciou!${NC}"
